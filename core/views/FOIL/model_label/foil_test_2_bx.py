import math,re,copy,json,time,heapq
    ...
    while (len(positive_list)> 0.4*initial_pos_length):
        ...
        while (len(negative_list)> 0.4*initial_neg_length):
            ...
            for new_clause in possible_clause:           #calculate the new possible clause foil_gain
                for rule in lock[c[0]]:
                    has=False
                    if new_clause in rule:
                        foil_gain_list.append(-99)
                        has=True
                        break
                if has==False:    
                    now_p=now_n=0
            ...
            for gain_num in range(len(foil_gain_list)):
                if foil_gain_list[gain_num]<heapq.nlargest(10, foil_gain_list)[9]:
                    foil_gain_list[gain_num]=-99

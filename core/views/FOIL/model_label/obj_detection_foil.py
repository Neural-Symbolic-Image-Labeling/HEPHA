...
def get_result_list(target,result_list,total_list):
    ...
    for image in result_list:
        for clauses in image:
            a=re.split(r'[(|,|)]',clauses)
            if a[0]!="overlap" and a[0]!="num" and a[0]!="area" and a[0]!="color":
...
def get_total_list1(input_list):
    ...
    for image_num in range(len(input_list)):
        for objs_num,objs in enumerate(input_list[image_num]['type']):
            image_list=[]
            string=objs['name'][0]+"(image"+str(input_list[image_num]['imageId'])+")"
...
def get_common(total_list):
    common_list=[]
    for image_num in range(len(total_list)):
        for predicate in total_list[image_num]:
            result=True
            for image in total_list:
                if predicate not in image:
                    result=False
                    break
            if result==True:
                common_list.append(predicate)
    return common_list

def change_result(result_list,common_list):
    results_list=[]
    for clause in result_list:
        for common_predicate in common_list:
            if common_predicate not in clause:
                clause.append(common_predicate)
        results_list.append(clause)
    return results_list
...
def foil(target_list,target,total_list,deleted,locked):         #target should be a string, such as "guitarist"
                ...
                for clause_number,clause_gain in enumerate(foil_gain_list):
                    ...
                    if clause_gain==max(foil_gain_list):
                        ...
                        if a[0]=="overlap":
                            ...
                            if a[1] in parameter_list and threshold(target,possible_clause[clause_number],total_list1)!=False and still_has_num(result)>=3:
...
def NL(result_list,target,total_list):
    ...
    for results in result_list:
        ...
        for i,clauses in enumerate(results):
            n=''
            a=re.split(r'[(|,|)]',clauses)
            if a[0]!='overlap' and a[0]!="num" and a[0]!='area' and a[0]!='color' and len(a)==4:
...
def obj_FOIL(input_list,deleted,locked):
    ...
    for target in target_list:
        ...
        #print(result_list)
        #print("This is result:",result_list)
        if result_list==None:
            dict_math[target]=[['none']]
            dict_nl[target]=[['none']]
        else:
            result_list=change_result(result_list,common_list)

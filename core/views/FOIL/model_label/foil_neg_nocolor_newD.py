...
def get_total_list1(input_list):
    ...
    for image_num in range(len(input_list)):
        ...
        position_list1=[]
        name_list=[]
        if not 'overlap' in input_list[image_num]['object_detect']:
            input_list[image_num]['object_detect']['overlap'] = {}
        if not 'panoptic_segmentation' in input_list[image_num]:
            input_list[image_num]['panoptic_segmentation'] = {}
        for objects_num in range(len(input_list[image_num]['object_detect']['object'])):
            name=input_list[image_num]['object_detect']['object'][str(objects_num)]['name']
            ...
            while correct_clause == False:
                for clause_number,clause_gain in enumerate(foil_gain_list):
                    ...
                    if clause_gain==max(foil_gain_list):
                        break_for=False
                        for rule in lock[c[0]]:
                            if possible_clause[clause_number] in rule:
...
def main():
    ...
    locked={"downtown": [["building(X,13)", "tree(X,9)"]], "highway": [], "mountain": []}
    dict_math,dict_nl,object_list,object_detection,segmentation=neg_FOIL(input_list,deleted,locked)
    print(dict_math)
main()